...
- **lock-free キュー検証。** MPSC mailbox と SPSC リング、および `ask`/`ask_timeout` の reply cell は
  Loom で検証(小モデルの全合法インターリーブを探索し順序/可視性を証明)、Miri が unsafe 経路の UB を
  カバー。モデルは意図的に極小なので順序規律を証明するものでキュー全体ではない:
  `RUSTFLAGS="--cfg aetherflow_loom"` で `cargo test --lib --release -p aetherflow`。
